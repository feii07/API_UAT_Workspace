...
function App(){
  ...
  useEffect(()=>{
    const h = (e:KeyboardEvent) => {
      ...
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'e') { e.preventDefault(); exportEvidence(); }
    };
    ...
    <div className="app">
      ...
      <main>
        <aside>
          ...
          {projects.map((p) => (
            <button className={'project ' + (project?.id === p.id ? 'sel' : '')} onClick={() => { setProject(p); loadScenarios(p.id); }} key={p.id}>{p.name}</button>
          ))}

          <div className="sideTitle scenarios">SCENARIOS</div>
          <input placeholder="Search" value={query} onChange={(e) => setQuery(e.target.value)} />
          {filtered.map((s) => (
            <button className={'scenario ' + (selected?.id === s.id ? 'sel' : '')} onClick={() => selectScenario(s)} key={s.id}>
        ...
        <section className="workspace">
          <div className="scenarioHead">
            ...
            {selected && <div className={'badge ' + (selected.status === 'PASS' ? 'pass' : selected.status === 'FAIL' ? 'fail' : '')}>{selected.status}</div>}
          </div>
          ...
          {tab === 'response' && (
            <div className="panel">
              ...
              <pre className="responseBox">{response ? JSON.stringify(JSON.parse(response.body || '{}'), null, 2) : ''}</pre>
              ...
              {attachments.length === 0 ? <div className="emptyState">No attachments yet.</div> : attachments.map((a) => (
                <div className="attachmentRow" key={a.id}>
                  <div className="attachmentMeta">
                    <label className="checkboxRow"><input type="checkbox" checked={selectedAttachmentIds.includes(a.id)} onChange={() => toggleAttachmentSelection(a.id)} /> {a.file_name}</label>
                    <small>{Math.round(a.size / 1024)} KB</small>
                  </div>
                  <textarea value={a.description} onChange={(e) => { const x=[...attachments]; const idx=x.findIndex(v=>v.id===a.id); if (idx >= 0) { x[idx].description=e.target.value; setAttachments(x); } }} placeholder="Describe why this attachment is relevant to UAT evidence." />
                  <div className="actionRow compact">
