...
struct Db(pub Mutex<Connection>);
#[derive(Serialize)] struct Project{id:i64,name:String,description:String}
#[derive(Serialize,Deserialize,Clone)] struct Param{enabled:bool,key:String,value:String,kind:String,description:String}
#[derive(Serialize,Deserialize,Clone)] struct Header{enabled:bool,key:String,value:String}
#[derive(Serialize,Deserialize,Clone)] struct RequestDef{method:String,url:String,params:Vec<Param>,headers:Vec<Header>,body:String,auth:String,token:String,basicUser:String,basicPass:String,apiKey:String}
#[derive(Serialize,Deserialize,Clone)] struct Rule{id:i64,target:String,operator:String,expected:String,enabled:bool,result:Option<bool>,actual:Option<Value>}
#[derive(Serialize)] struct Attachment{id:i64,file_name:String,description:String,size:i64,relative_path:String,created_at:String}
...
fn db(_app:&AppHandle)->Result<Connection,String>{let exe=std::env::current_exe().map_err(|e|e.to_string())?; let root=exe.parent().ok_or("Cannot locate application directory")?.join("data"); fs::create_dir_all(&root).map_err(|e|e.to_string())?; let p=root.join("workspace.db"); let c=Connection::open(p).map_err(|e|e.to_string())?; init(&c)?; Ok(c)}
...
#[tauri::command] fn inspect_excel(path:String)->Result<Value,String>{let wb=open_workbook_auto(&path).map_err(|e|format!("Excel import failed: {e}"))?;let sheets=wb.sheet_names().to_vec();Ok(json!({"sheets":sheets}))}
...
fn value_at(body:&Value,target:&str)->Option<Value>{let parts:Vec<&str>=target.split('.').collect();if parts.len()<2{return None}let mut v=body;for p in parts.iter().skip(1){v=v.get(*p)?;}Some(v.clone())}
fn actual(resp:&Resp,target:&str)->Option<Value>{if target=="response.status"{return Some(json!(resp.status))}if target.starts_with("body."){let v:Value=serde_json::from_str(&resp.body).ok()?;return value_at(&v,target)}None}
fn compare(a:&Option<Value>,op:&str,e:&str)->bool{match op{"exists"=>a.is_some(),"not exists"=>a.is_none(),"null"=>a==&Some(Value::Null),"not null"=>a.is_some()&&a!=&Some(Value::Null),"empty"=>a.as_ref().map(|v|v.as_str().map(|s|s.is_empty()).unwrap_or(false)||v.as_array().map(|x|x.is_empty()).unwrap_or(false)).unwrap_or(false),"not empty"=>!compare(a,"empty",e),"type is"=>a.as_ref().map(|v|match e.to_lowercase().as_str(){"string"=>v.is_string(),"number"=>v.is_number(),"boolean"=>v.is_boolean(),"array"=>v.is_array(),"object"=>v.is_object(),"null"=>v.is_null(),_=>false}).unwrap_or(false),_=>{let av=a.as_ref().map(|v|if v.is_string(){v.as_str().unwrap().to_string()}else{v.to_string()}).unwrap_or_default();match op{"equals"=>av==e,"not equal"=>av!=e,"contains"=>av.contains(e),"not contains"=>!av.contains(e),"starts with"=>av.starts_with(e),"ends with"=>av.ends_with(e),"greater"=>av.parse::<f64>().ok().zip(e.parse::<f64>().ok()).map(|(a,b)|a>b).unwrap_or(false),"greater or equal"=>av.parse::<f64>().ok().zip(e.parse::<f64>().ok()).map(|(a,b)|a>=b).unwrap_or(false),"less"=>av.parse::<f64>().ok().zip(e.parse::<f64>().ok()).map(|(a,b)|a<b).unwrap_or(false),"less or equal"=>av.parse::<f64>().ok().zip(e.parse::<f64>().ok()).map(|(a,b)|a<=b).unwrap_or(false),"regex"=>Regex::new(e).map(|r|r.is_match(&av)).unwrap_or(false),_=>false}}}}
#[tauri::command] fn validate_response(state:State<Db>,scenario_id:i64,response:Resp,rules:Vec<Rule>)->Result<Value,String>{let c=state.0.lock().unwrap();let mut out=Vec::new();for r in rules{let a=actual(&response,&r.target);let ok=!r.enabled||compare(&a,&r.operator,&r.expected);out.push(json!({"id":r.id,"target":r.target,"operator":r.operator,"expected":r.expected,"enabled":r.enabled,"result":ok,"actual":a}));}let final_result=if out.is_empty(){"NOT VALIDATED"}else if out.iter().all(|x|x["result"]==true){"PASS"}else{"FAIL"};for x in &out{let rid=x["id"].as_i64().unwrap_or(0);if rid>0{c.execute("UPDATE validation_rules SET target=?,operator=?,expected=?,enabled=? WHERE id=?",params![x["target"].as_str().unwrap_or(""),x["operator"].as_str().unwrap_or(""),x["expected"].as_str().unwrap_or(""),x["enabled"].as_bool().unwrap_or(true),rid]).ok();}else{c.execute("INSERT INTO validation_rules(scenario_id,target,operator,expected,enabled) VALUES(?,?,?,?,?)",params![scenario_id,x["target"].as_str().unwrap_or(""),x["operator"].as_str().unwrap_or(""),x["expected"].as_str().unwrap_or(""),x["enabled"].as_bool().unwrap_or(true)]).ok();}}let _=scenario_id;Ok(json!({"rules":out,"final_result":final_result}))}
fn redact_headers(h:&[Header])->HashMap<String,String>{h.iter().filter(|x|x.enabled).map(|x|(x.key.clone(),mask(&x.key,&x.value))).collect()}
