...
#[tauri::command] fn validate_response(state:State<Db>,scenario_id:i64,response:Resp,rules:Vec<Rule>)->Result<Value,String>{let c=state.0.lock().unwrap();let mut out=Vec::new();for r in rules{let a=actual(&response,&r.target);let ok=!r.enabled||compare(&a,&r.operator,&r.expected);out.push(json!({"id":r.id,"target":r.target,"operator":r.operator,"expected":r.expected,"enabled":r.enabled,"result":ok,"actual":a}));}let final_result=if out.is_empty(){"NOT VALIDATED"}else if out.iter().all(|x|x["result"]==true){"PASS"}else{"FAIL"};for x in &out{let rid=x["id"].as_i64().unwrap_or(0);if rid>0{c.execute("UPDATE validation_rules SET target=?,operator=?,expected=?,enabled=? WHERE id=?",params![x["target"],x["operator"],x["expected"],x["enabled"].as_bool().unwrap_or(true),rid]).ok();}else{c.execute("INSERT INTO validation_rules(scenario_id,target,operator,expected,enabled) VALUES(?,?,?,?,?)",params![scenario_id,x["target"],x["operator"],x["expected"],x["enabled"].as_bool().unwrap_or(true)]).ok();}}let _=scenario_id;Ok(json!({"rules":out,"final_result":final_result}))}
